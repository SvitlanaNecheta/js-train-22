...
class AuthProcessor {
  ...
  validate(username,passkey){
    if(this.nextProcessor){return this.nextProcessor.validate(username, passkey);}else{return false;}
  }
...
class TwoStepProcessor extends AuthProcessor {
  validate(username,passkey){
    if((username==="john") && (passkey==="password")){
      ...
      return true;}else{return super.validate(username, passkey)}
  }
  ...
  isValidTwoStepCode(){return true} // isValidTwoStepCode Метод для перевірки двофакторного коду,який повертає true.
}
...
class ProcessorBuilder {
 ...
 add(processor){
  if(!this.firstProcessor){this.firstProcessor=processor;
  this.lastProcessor=processor;}else{
