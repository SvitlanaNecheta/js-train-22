...
class Participant {
  ...
  dispatchMessage(text){ const prepare=this.prepareMessage(text);
   this.communicator.transmit(prepare);
  ...
  prepareMessage(text){return `[${this.alias}]: ${text}`}// Метод prepareMessage приймає text та повертає  `[${this.alias}]: ${text}`
}

// Клас SMSCommunicator відповідає за відправку повідомлень через SMS.
class SMSCommunicator {
 static transmit(message){console.log(`Відправлено SMS: ${message}`);}; // Статичний метод transmit відправляє SMS.
...
class EmailCommunicator {
  static transmit(message){console.log(`Відправлено Email: ${message}`)}// Статичний метод transmit відправляє Email.
