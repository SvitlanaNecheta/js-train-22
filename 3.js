...
class Customer {
  ...
  sendEmail(message){console.log(`Відправки на ${this.email}, повідомлення: ${message}`)}
  /**
...
class Store {
   ...
   */
  unsubscribe(customer){this.customers=this.customers.filter((el)=>el!==customer);}
  /**
  ...
  sendNotify(product){
    this.customers.forEach((customer)=>{customer.sendEmail(`Новий продукт "${product.name}" в магазині ${this.name}!`);});
  } 
  /**
